  ...
  const handleSaveVisit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        recording_url: recordingStoragePath || null,
        recording_name: recordingStoragePath ? fileName : null,
        recording_duration: duration > 0 ? duration : null,
        status: recordingStoragePath ? 'recording' as const : 'preparing' as const,
      };

      let visitId = savedVisitId;
      if (savedVisitId) {
        await supabase.from('visits').update(payload).eq('id', savedVisitId);
      } else {
        const { data } = await supabase.from('visits').insert(payload).select('id').maybeSingle();
