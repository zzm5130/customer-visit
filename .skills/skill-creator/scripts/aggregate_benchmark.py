...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines.append(
        f"| Pass Rate | "
        f"{a_pr.get('mean', 0)*100:.0f}% ± {a_pr.get('stddev', 0)*100:.0f}% | "
    ...
    lines.append(
        f"| Time | "
        f"{a_time.get('mean', 0):.1f}s ± {a_time.get('stddev', 0):.1f}s | "
    ...
    lines.append(
        f"| Tokens | "
        f"{a_tokens.get('mean', 0):.0f} ± {a_tokens.get('stddev', 0):.0f} | "
...
def main():
    """Parse CLI arguments and generate benchmark JSON and markdown files."""
