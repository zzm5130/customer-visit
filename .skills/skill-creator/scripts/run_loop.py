    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if verbose:
            def print_eval_stats(label, results, elapsed):
                """Print precision/recall/accuracy stats and per-query results for a label."""
...
def main():
    """Parse CLI arguments and run the eval+improve loop, saving results."""
    ...
    if args.report != "none":
        if args.report == "auto":
            ...
            report_filename = f"skill_description_report_{skill_path.name}_{timestamp}.html"
            live_report_path = Path(tempfile.gettempdir()) / report_filename
        else:
        ...
        live_report_path.write_text(placeholder)
